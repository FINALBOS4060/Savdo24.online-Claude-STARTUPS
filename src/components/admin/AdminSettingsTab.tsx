            ...
            return (
              <div key={key} className="bg-surface-container border border-white/5 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1 md:w-1/3">
                  <span className="text-xs font-bold font-mono text-secondary">{key}</span>
                  {settingsStatus[key] && (
                    <p className="text-xs text-success font-semibold">{settingsStatus[key]}</p>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-1">
                  <input
                    ...
                    className="w-full p-2.5 bg-surface-container-low border border-white/10 rounded-xl text-white font-mono text-xs focus:outline-none focus:ring-2 focus:ring-secondary"
