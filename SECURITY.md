...
- **CSP Style Security:** `helmet` konfiguratsiyasida `styleSrc` uchun `unsafe-inline` saqlab qolindi. Bu, Material Symbols'ning `font-variation-settings` xususiyatini ishlatish uchun zarur. Inline stillar kod bazasida juda kam (asosan Material Symbols uchun ishlatiladi), boshqa barcha stillar Tailwind CSS klasslari orqali boshqariladi.
